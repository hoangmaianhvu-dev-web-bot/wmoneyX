...
type GameTab = 'daily' | 'blindbag' | 'wheel' | 'vip';
    ...
    <div className="max-w-4xl mx-auto space-y-6 p-6 pb-24">
      ...
      <div className="flex gap-2 bg-black/40 p-1 rounded-2xl border border-white/5 overflow-x-auto no-scrollbar">
        ...
        <button 
          ...
          className={`flex-1 min-w-[100px] py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${activeTab === 'vip' ? 'bg-accent text-black shadow-lg' : 'text-gray-500 hover:text-white'}`}
        >
          <Trophy size={14} /> Đua Top VIP
        </button>
      </div>
      ...
      <AnimatePresence mode="wait">
        ...
        {activeTab === 'vip' && (
          ...
          >
            <div className="text-center space-y-2">
              <h2 className="text-2xl font-black uppercase tracking-[0.2em] ocean-glow">Đua Top VIP</h2>
              <p className="text-[10px] text-gray-500 uppercase font-bold tracking-widest">Bảng xếp hạng dựa trên tổng EXP</p>
            </div>

            <div className="glass rounded-[2rem] overflow-hidden">
              <div className="divide-y divide-white/5">
                {vipLeaderboard.length > 0 ? (
                  vipLeaderboard.map((user, index) => (
                    <div 
                      key={user.id} 
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-2">
                          <span className={`px-2 py-0.5 rounded-md font-black text-[8px] uppercase ${user.vip_status === 'VIP GOLD' ? 'bg-yellow-400/10 text-yellow-400' : 'bg-zinc-800 text-zinc-500'}`}>
                            {user.vip_status || 'MEMBER'}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="text-right">
                        <div className="flex items-center gap-1 justify-end text-accent">
                          <Zap size={10} />
                          <span className="text-[10px] font-black">{user.exp?.toLocaleString() || 0} EXP</span>
                        </div>
                      </div>
                    </div>
                  ))
