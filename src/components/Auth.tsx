  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${redirectUrl}${referralCode ? '?ref=' + referralCode : ''}`,
        }
      });
            ...
            {mode !== 'forgot' && (
              <>
                ...
                <button 
                  ...
                  disabled={loading}
                  className="w-full py-4 bg-white text-gray-900 rounded-2xl font-bold text-xs uppercase tracking-widest hover:bg-gray-100 transition-all flex items-center justify-center gap-3 group shadow-lg"
                >
