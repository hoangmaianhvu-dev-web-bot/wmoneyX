...
const Tasks: React.FC<TasksProps> = ({ balance, userId, profile, onBack, onUpdateBalance, onUpdateProfile }) => {
  ...
  const handleSuccess = async (isSpecialTask: boolean = false) => {
    try {
      ...
      const taskReward = isSpecialTask ? 1000 : reward;
          ...
          <div className="flex items-start gap-5">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-3 text-[10px] mb-4">
                <span className="text-red-400 font-bold">Thưởng: 1000 Xu</span>
              ...
              <button 
                onClick={async () => {
                  if (!captchaToken) {
                    showNotification({
                      title: "LỖI XÁC MINH",
                      message: "Vui lòng hoàn thành hCaptcha trước khi làm nhiệm vụ đặc biệt.",
                      type: "error"
                    });
                    return;
                  }
                  ...
                  try {
                    ...
                    if (!data.success) {
                      throw new Error('Xác minh hCaptcha thất bại');
                    }
                    ...
                    showNotification({
                      title: "LỖI",
                      message: "Xác minh hCaptcha thất bại. Vui lòng thử lại.",
                      type: "error"
                    });
                  }
                }}
            ...
            >
              <div className="text-center mb-6">
                ...
                <p className="text-[10px] text-accent uppercase font-bold mt-2">
                  Thường: 200 Xu / Đặc biệt: 1000 Xu
                </p>
